...
const _processPage = (async (convertAction, uriOrHtml, options) => {
  ...
  try {
    ...
    if (debugLogFile) {
      ...
      fs.appendFile(debugLogFile, JSON.stringify(["ok", debugData]) + "\n", (err) => ( console.error (err) ));
    }
...
function _handleError(error) {
  ...
  if (error instanceof Error) {
    stringfiedError = JSON.stringify(['err', error.toString().replace(new RegExp('^' + error.name + ': '), ''), error.name]);
  } else {
    stringfiedError = JSON.stringify(['err', JSON.stringify(error)]);
  }

  if (debugLogFile) fs.appendFile(debugLogFile, stringfiedError + "\n", (err) => ( console.error (err) ));

  process.stdout.write(stringfiedError);
  process.stdout.write("\n");
}
...
require('readline').createInterface({
  ...
  try {
    options = JSON.parse(line)[2];
    debugLogFile = options.debugLogFile; delete options.debugLogFile;

    Promise.resolve(_processPage.apply(null, JSON.parse(line)))
      .then(function (result) {
        let stringfiedResult = JSON.stringify(['ok', result]);
        if (debugLogFile) fs.appendFile(debugLogFile, stringfiedResult + "\n", (err) => ( console.error (err) ));
        process.stdout.write(stringfiedResult);
        process.stdout.write("\n");
      })
      .catch(_handleError);
  } catch(error) {
